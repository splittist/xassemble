...
export function QuestionnairePage() {
  ...
  useEffect(() => {
    api<Questionnaire>(`/document-sets/${slug}/questionnaire`)
      .then((value) => { setQuestionnaire(value); setVisible(value.questions.map((question) => question.variable_name)); })
      .catch((caught) => setError(caught instanceof Error ? caught.message : "Could not open this questionnaire."));
  }, [slug]);

  useEffect(() => {
    if (!questionnaire) return;
    const timer = window.setTimeout(() => {
      api<{ visible: string[]; hidden: string[] }>(`/document-sets/${slug}/questionnaire/evaluate`, { method: "POST", body: JSON.stringify({ answers }) })
        .then((result) => setVisible(result.visible))
        .catch((caught) => setError(caught instanceof Error ? caught.message : "Could not update the questionnaire."));
    }, 250);
  ...
  async function generate(event: FormEvent) {
    ...
    try { await download(`/document-sets/${slug}/generate`, { method: "POST", body: JSON.stringify({ answers, project_code: projectCode }) }); }
  ...
  return (
    <form className="questionnaire-layout" onSubmit={generate}>
      <div className="questionnaire-main">
        <BackLink />
        <header className="page-heading compact"><div><p className="eyebrow">New assembly</p><h1>{humanize(slug)}</h1><p>Complete the visible questions. Your answers stay in this browser until generation.</p></div></header>
      ...
      <aside className="generation-panel">
        ...
        <button className="button primary generate-button" disabled={generating}><Download size={17} /> {generating ? "Generating…" : "Generate documents"}</button>
